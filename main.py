...
def write_pgm(data, fname):
  with open(fname, "wb") as f:
    ...
    f.write(("%d %d\r\n" % (width, height)).encode())
    f.write(b"65535\r\n")
    f.write(numpy.asarray(data, dtype='>i2').tobytes())
...
if __name__ == "__main__":
  ...
  data = numpy.zeros(width * height, dtype='>i2')
  idx = 0
  ...
  # Now, data contains the julia set as an array, write to a file.
  write_pgm(data, "julia_fractal.pgm")
  print(f"elapsed time: {clock() - start:.1f} seconds", file=sys.stderr)
  print(f"second largest count: {second_largest}", file=sys.stderr)
